...
def display(confusion_matrix, categories, output_path):
    ...
    for i in range(len(categories)):
        ...
        accuracy = float((confusion_matrix[0, 0] + confusion_matrix[1, 1] + confusion_matrix[2 , 2])/total)
        
        #print('precision_{}@{}IOU: {:.2f}'.format(name, IOU_THRESHOLD, precision))
        #print('recall_{}@{}IOU: {:.2f}'.format(name, IOU_THRESHOLD, recall))
        #print('accuracy{}@{}IOU: {:.2f}'.format(name, IOU_THRESHOLD, accuracy))
        
        results.append({'category' : name, 'm_precision_@{}IOU'.format(IOU_THRESHOLD) : precision, 'm_accuracy_@{}IOU'.format(IOU_THRESHOLD) : accuracy, 'm_recall_@{}IOU'.format(IOU_THRESHOLD) : recall})
